...
class CheckContext:
    ...
    via Makefile, or from tofu output when run interactively.
    """
    ...
    def from_env(cls) -> "CheckContext":
        ...
        ctx = cls(
            talosctl=shutil.which("talosctl") or "talosctl",
            kubectl=shutil.which("kubectl") or "kubectl",
            talosconfig=os.environ.get("CHECK_TALOSCONFIG", "talosconfig"),
            kubeconfig=os.environ.get("CHECK_KUBECONFIG", "kubeconfig"),
            endpoint=os.environ.get("CHECK_ENDPOINT", ""),
            node=os.environ.get("CHECK_NODE", ""),
            talos_version=os.environ.get("CHECK_TALOS_VERSION", ""),
            ...
            == "true",
            zfs_pool_name=os.environ.get("CHECK_ZFS_POOL_NAME", "tank"),
            public_ip=os.environ.get("CHECK_PUBLIC_IP", ""),
            tailscale_ip=os.environ.get("CHECK_TAILSCALE_IP", ""),
    ...
    def from_tofu(cls) -> "CheckContext":
        ...
        if not ctx.talos_version:
            ...
            if ":" in installer:
                ctx.talos_version = installer.rsplit(":", 1)[-1]
        ctx.public_ip = ctx.public_ip or _get(outputs, "server_ip", "")
        ...
        tailscale_ip = _get(outputs, "tailscale_device_ip", "")
        if tailscale_ip:
        ...
        if isinstance(zfs_info, dict):
            ...
            ctx.zfs_pool_name = zfs_info.get("pool_name", ctx.zfs_pool_name)
