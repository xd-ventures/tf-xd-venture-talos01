...
- Boolean flags: `{thing}_enabled` (e.g., `argocd_enabled`, `zfs_pool_enabled`)
- Network: `{network}_cidr`, `{network}_ip`
- Grandfathered exceptions (pre-v0.1.0 names kept to avoid breaking operator
  tfvars; rename only at the next major version): `enable_firewall`,
  `use_raw_image`, `ephemeral_max_size` (lacks its `zfs_pool_` prefix).
  Do not model new variables on these.
...
- Never shadow a variable name with an output whose value differs — disambiguate
  (e.g. output `tailscale_fqdn` vs var `tailscale_hostname`, output
  `cluster_endpoint_actual` vs var `cluster_endpoint`, output
  `server_state_actual` vs var `server_state`)
